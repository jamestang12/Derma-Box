...
class AcneVulgarisModel:
    ...
    def load_from_disk(self):
        ...
        """loss = 1
        gradients, variables = zip(my_optimizer.compute_gradients(loss))
        gradients = [
            None if gradient is None else tf.clip_by_norm(gradient, 5.0)
            for gradient in gradients]
        my_optimizer = my_optimizer.apply_gradients(zip(gradients, variables))"""

        # my_optimizer = tf.estimator.Estimator.clip_gradients_by_norm(my_optimizer, 5.0)
